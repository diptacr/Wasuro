...
uses
    types, leb128, wasm.test.framework;
...
begin
    ...
    assert_u32('u32 value=0 bytes=1', uint32(bytesRead), 1);

    { Test: single byte, value 42 }
    buf[0] := $2A;
    bytesRead := read_leb128_to_uint32(@buf[0], buf_end, @res32);
    assert_u32('u32 value=42', res32, 42);
    assert_u32('u32 value=42 bytes=1', uint32(bytesRead), 1);
    ...
    assert_u32('u32 value=128 bytes=2', uint32(bytesRead), 2);
    ...
    assert_u32('u32 value=624485 bytes=3', uint32(bytesRead), 3);

    { Test: uint64 single byte }
    buf[0] := $2A;
    bytesRead := read_leb128_to_uint64(@buf[0], buf_end, @res64);
    assert_u64('u64 value=42', res64, 42);
    assert_u32('u64 value=42 bytes=1', uint32(bytesRead), 1);

    { Test: uint64 two bytes }
    buf[0] := $80;
    buf[1] := $01;
    bytesRead := read_leb128_to_uint64(@buf[0], buf_end, @res64);
    assert_u64('u64 value=128', res64, 128);

    { Test: uint64 multi-byte }
    buf[0] := $E5;
    buf[1] := $8E;
    buf[2] := $26;
    bytesRead := read_leb128_to_uint64(@buf[0], buf_end, @res64);
