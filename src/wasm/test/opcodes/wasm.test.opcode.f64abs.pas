...
begin
    test_begin('opcode.f64.abs');

    code[0] := $99;
    ctx := make_test_context(@code[0], 1);
    pushf64(ctx^.ExecutionState.Operand_Stack, 3.5);
    wasm.vm.tick(ctx);
    assert_f64('abs(3.5)=3.5', popf64(ctx^.ExecutionState.Operand_Stack), 3.5);
    ...
    wasm.vm.tick(ctx);
    assert_f64('abs(0.0)=0.0', popf64(ctx^.ExecutionState.Operand_Stack), 0.0);
    ...
    assert_true('abs(NaN)=NaN', ((resultBits and $7FF0000000000000) = $7FF0000000000000) and ((resultBits and $000FFFFFFFFFFFFF) <> 0));
