...
begin
    ...
    wasm.vm.tick(ctx);
    assert_f64('max(1.0,2.0)=2.0', popf64(ctx^.ExecutionState.Operand_Stack), 2.0);
    ...
    pushf64(ctx^.ExecutionState.Operand_Stack, 1.0);
    wasm.vm.tick(ctx);
    assert_f64('max(-1.0,1.0)=1.0', popf64(ctx^.ExecutionState.Operand_Stack), 1.0);
    ...
    pushf64(ctx^.ExecutionState.Operand_Stack, 3.0);
    wasm.vm.tick(ctx);
    assert_f64('max(3.0,3.0)=3.0', popf64(ctx^.ExecutionState.Operand_Stack), 3.0);
    ...
    resultVal := popf64(ctx^.ExecutionState.Operand_Stack);
    resultBits := TWASMPUInt64(@resultVal)^;
    assert_u64('max(-0,+0)=+0', resultBits, $0000000000000000);
