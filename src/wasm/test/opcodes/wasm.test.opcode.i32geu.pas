...
begin
    ...
    { Test: 0 >= -1 (0xFFFFFFFF) -> 0 (unsigned: 0 not >= max) }
    code[0] := $4F;
    ctx := make_test_context(@code[0], 1);
    pushi32(ctx^.ExecutionState.Operand_Stack, 0);
    pushi32(ctx^.ExecutionState.Operand_Stack, -1);
    wasm.vm.tick(ctx);
    assert_i32('0<0xFFFFFFFF', popi32(ctx^.ExecutionState.Operand_Stack), 0);
