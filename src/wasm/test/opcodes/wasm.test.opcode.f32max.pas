...
begin
    ...
    wasm.vm.tick(ctx);
    assert_f32('max(1.0,2.0)=2.0', popf32(ctx^.ExecutionState.Operand_Stack), 2.0);
    ...
    pushf32(ctx^.ExecutionState.Operand_Stack, 1.0);
    wasm.vm.tick(ctx);
    assert_f32('max(-1.0,1.0)=1.0', popf32(ctx^.ExecutionState.Operand_Stack), 1.0);
    ...
    pushf32(ctx^.ExecutionState.Operand_Stack, 3.0);
    wasm.vm.tick(ctx);
    assert_f32('max(3.0,3.0)=3.0', popf32(ctx^.ExecutionState.Operand_Stack), 3.0);
    ...
    assert_u32('max(NaN,1.0) is NaN', resultBits and $7FC00000, $7FC00000);
    ...
    resultVal := popf32(ctx^.ExecutionState.Operand_Stack);
    resultBits := TWASMPUInt32(@resultVal)^;
    assert_u32('max(-0,+0)=+0', resultBits, $00000000);

    { max(+0.0, -0.0) = +0.0 (commutative) }
    code[0] := $97;
    ctx := make_test_context(@code[0], 1);
    pushf32(ctx^.ExecutionState.Operand_Stack, 0.0);
    pushf32(ctx^.ExecutionState.Operand_Stack, TWASMPFloat(@negZero)^);
