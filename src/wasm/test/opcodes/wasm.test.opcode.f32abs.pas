...
begin
    test_begin('opcode.f32.abs');

    code[0] := $8B;
    ctx := make_test_context(@code[0], 1);
    pushf32(ctx^.ExecutionState.Operand_Stack, 3.5);
    wasm.vm.tick(ctx);
    assert_f32('abs(3.5)=3.5', popf32(ctx^.ExecutionState.Operand_Stack), 3.5);
    ...
    wasm.vm.tick(ctx);
    assert_f32('abs(0.0)=0.0', popf32(ctx^.ExecutionState.Operand_Stack), 0.0);
    ...
    assert_true('abs(NaN)=NaN', ((resultBits and $7F800000) = $7F800000) and ((resultBits and $007FFFFF) <> 0));
